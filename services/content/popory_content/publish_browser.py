...
_SETUP_SIGNALS = (
    # 모델이 백그라운드 브라우저 작업의 알림을 기다리겠다며 응답을 끝낸 경우. --print 세션은 알림을 못 받는다.
    ("알림을 받", "브라우저 작업 완료 전에 응답을 끝냄(비대화형 세션은 완료 알림을 받지 못한다) — 글은 올라갔을 수 "
                "있으니 블로그 글 목록·임시저장을 먼저 확인. 프롬프트의 폴링 지시가 최신인지 확인"),
    ("폴링 없이", "브라우저 작업 완료 전에 응답을 끝냄(비대화형 세션은 완료 알림을 받지 못한다) — 글은 올라갔을 수 "
                "있으니 블로그 글 목록·임시저장을 먼저 확인. 프롬프트의 폴링 지시가 최신인지 확인"),
    ("완료되는 대로", "브라우저 작업 완료 전에 응답을 끝냄(비대화형 세션은 완료 알림을 받지 못한다) — 글은 올라갔을 수 "
                   "있으니 블로그 글 목록·임시저장을 먼저 확인. 프롬프트의 폴링 지시가 최신인지 확인"),
...
- **이 세션은 비대화형 단일 턴입니다.** 응답을 끝내는 순간 프로세스가 종료되고, 그 뒤에 오는 알림·완료 통지는 **절대 전달되지 않습니다.** 브라우저 작업(`exec`)이 백그라운드 작업 ID 만 돌려주면 "알림을 기다리겠다" 고 응답을 끝내지 말고, 그 작업의 상태 조회·대기 도구를 **완료될 때까지 반복 호출(폴링)** 해 최종 결과를 손에 쥔 뒤에만 답합니다. "완료되면 보고드리겠습니다" 로 끝난 응답은 결과 미보고 실패이며, 글은 올라갔는데 시스템은 모르는 최악의 상태를 만듭니다(2026-09-07 발생).
...
def build_instructions(task: dict[str, Any], body_path: Path) -> str:
    ...
    head = [f"/{BROWSER_SKILL}", "",
            ...
            "주의: 이 세션은 비대화형 단일 턴이라 응답을 끝내면 끝입니다. 브라우저 작업이 백그라운드로 돌면 "
            "완료될 때까지 상태를 반복 조회(폴링)하고, 최종 결과를 확인한 뒤 <publish_result> 태그로 답합니다. "
            "\"완료되면 알림을 받겠다\" 고 응답을 끝내지 마십시오.", ""]
    if kind == "naver":
        steps = [
...
def publish(task: dict[str, Any], *, runner=None, call=run_claude_cli_once) -> dict[str, Any]:
    ...
    try:
        ...
        if "publish_result 태그 없음" in msg:
            ...
            # 출력 꼬리에서 원인(알림 대기로 턴 종료 등)을 읽어 한 줄 붙인다.
            hint = _setup_hint(msg)
            cause = f" | 원인: {hint}" if hint else ""
            return {"status": "failed",
                    "error": ("결과 미보고 — 글이 이미 올라갔을 수 있습니다. 블로그의 글 목록·임시저장을 "
                              f"먼저 확인한 뒤 재시도하세요.{cause} {msg}")[:600]}
