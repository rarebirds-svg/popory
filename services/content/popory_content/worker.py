...
# 모델명은 이 파일이 아니라 workers/api 의 content_ai_image 라우트에 있다(워커는 프롬프트만 보낸다).
# 아래 4006/neurons 감지는 무료 티어 전용이다 — unit-priced 모델(klein 등)로 갈아타면 4006 이
# 안 떠서 폴백이 죽고 그대로 과금된다. 그게 a4a599b 에서 klein 교체를 되돌린 이유다.
...
def _try_cloudflare(client, prompt: str) -> bytes | None:
    """CF flux-schnell로 1장(모델은 포털 라우트가 정한다). 한도(4006/neurons)면 그날 소진
    표시 후 None(→로컬 폴백). 그 외 실패도 None."""
    try:
