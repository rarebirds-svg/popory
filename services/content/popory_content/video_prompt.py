...
- image_prompt는 자연스러운 실사 장면을 묘사합니다. **사람의 얼굴이 정면으로 또렷이 보이는 구도는 피합니다**(이미지 모델이 정면 얼굴을 기형·무서운 표정으로 그리기 쉽기 때문). 가능하면 **풍경·사물·상징적 이미지** 중심으로 장면을 표현하고, 사람이 필요하면 **뒷모습·옆모습·실루엣·역광, 또는 얼굴이 작게 보이는 충분히 먼 거리**로 담습니다. 얼굴 클로즈업·정면 응시·여러 사람의 얼굴 군집은 쓰지 않습니다. 인체는 해부학적으로 정상이고 온전하게(잘리거나 기형이 되지 않게) 표현합니다. (예: "a person walking away through a misty forest, seen from behind, soft backlight" 처럼 얼굴이 드러나지 않는 구도.)
...
- image_prompt는 자연스러운 실사 장면을 묘사합니다. **사람의 얼굴이 정면으로 또렷이 보이는 구도는 피합니다**(정면 얼굴이 기형·무서운 표정으로 그려지기 쉬움). 풍경·사물·상징 중심으로 담고, 사람이 필요하면 **뒷모습·실루엣·역광·원경**으로 얼굴이 드러나지 않게 합니다. 얼굴 클로즈업·정면 응시는 쓰지 않습니다. 인체는 해부학적으로 정상이고 온전하게 표현합니다.
