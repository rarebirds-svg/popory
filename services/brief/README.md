...
목적지가 죽은 주소와 **정확히 같을 때만** 벗긴다(`…/a` 가 죽어도 `…/a?b=1` 은 그대로).
텍스트에 대괄호가 있는 링크(`[[단독] 제목](url)`)와 `<URL>` 목적지도 같게 처리한다.
텍스트가 URL 자체인 링크(`[URL](URL)`, `<URL>`, 스킴·www 만 다른 표기)는 표기만 벗기면
맨 URL 이 남아 메일·포털 렌더러가 다시 링크로 만든다. 그래서 그 URL 은 코드 표기
(`` `URL` ``)로 바꿔 보이되 눌리지 않게 한다. 점검도 링크 목적지를 적힌 그대로 한다 —
문장 끝 구두점은 맨 URL 에서만 뗀다(`…/Apple_Inc.` 를 `…/Apple_Inc` 로 점검하지 않는다).
